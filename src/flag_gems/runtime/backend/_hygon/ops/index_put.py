...
def generate_index_put_kernel(
    ...
    with code.indent():
        ...
        args += [
            "M,",
            "N,",
            "BYTES_PER_ELEM: tl.constexpr,",
            "IS_ACCUMULATE: tl.constexpr,",
            "BLOCK_SIZE0: tl.constexpr = 128,",
            "BLOCK_SIZE1: tl.constexpr = 32,",
        ]
        code.writelines(args)
    code.writeline("):")

    with code.indent():
        code.writeline("input_addr_base = input_ptr.to(tl.uint64)")
        code.writeline("values_addr_base = values_ptr.to(tl.uint64)")
        for i in range(indices_len):
            code.writeline(f"indices{i}_addr_base = indices{i}_ptr.to(tl.uint64)")
        ...
        for i in range(indices_len):
            ...
            if not comp:
                ...
                offset_expr = " + ".join(comp)

            code.writeline(f"idx_offset_bytes_{i} = ({offset_expr}) * 8")
            code.writeline(
                f"idx_addr_{i} = indices{i}_addr_base + idx_offset_bytes_{i}"
            )
            code.writeline(
                f"idx_addr_safe_{i} = tl.where(mask0, idx_addr_{i}, indices{i}_addr_base)"
            )
            code.writeline(
                f"idx_ptr_{i} = idx_addr_safe_{i}.to(tl.pointer_type(tl.int64))"
            )
            code.writeline(f"cur_index{i} = tl.load(idx_ptr_{i}, mask=mask0, other=0)")
        ...
        if not comp:
            code.writeline("input_offset_elem = tl.zeros([1, 1], tl.int64)")
        else:
            code.writeline(f"input_offset_elem = {' + '.join(comp)}")

        code.writeline("input_offset_bytes = input_offset_elem * BYTES_PER_ELEM")
        code.writeline("final_input_addr = input_addr_base + input_offset_bytes")
        code.writeline(
            "final_input_addr_safe = tl.where(mask, final_input_addr, input_addr_base)"
        )

        code.writeline("final_input_ptr = final_input_addr_safe.to(input_ptr.dtype)")
        ...
        if not comp:
            code.writeline("values_offset_elem = tl.zeros([1, 1], tl.int64)")
        else:
            code.writeline(f"values_offset_elem = {' + '.join(comp)}")

        code.writeline("values_offset_bytes = values_offset_elem * BYTES_PER_ELEM")
        code.writeline("final_values_addr = values_addr_base + values_offset_bytes")
        code.writeline(
            "final_values_addr_safe = tl.where(mask, final_values_addr, values_addr_base)"
        )
        code.writeline("final_values_ptr = final_values_addr_safe.to(values_ptr.dtype)")

        code.newline()
        code.writeline("cur_value = tl.load(final_values_ptr, mask=mask)")
        code.writeline("if IS_ACCUMULATE:")
        with code.indent():
            code.writeline("tl.atomic_add(final_input_ptr, cur_value, mask=mask)")
        code.writeline("else:")
        with code.indent():
            code.writeline("tl.store(final_input_ptr, cur_value, mask=mask)")
...
def generate_index_put_wrapper(
    ...
    with code.indent():
        ...
        with code.indent():
            ...
            args += ["M,", "N,", "element_size,", "accumulate==True,"]
            code.writelines(args)
        code.writeline(")")
