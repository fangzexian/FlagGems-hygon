...
def generate_index_kernel(
    ...
    with code.indent():
        ...
        # Note: These strides will be passed as BYTE strides
    ...
    with code.indent():
        # Cast pointers to uint64 for manual byte addressing
        code.writeline("input_ptr = input_ptr.to(tl.uint64)")
        code.writeline("out_ptr = out_ptr.to(tl.uint64)")

        code.writeline("pid0 = tle.program_id(axis=0)")
        code.writeline("pid1 = tle.program_id(axis=1)")

        # Force int64 arithmetic for offsets to prevent 32-bit overflow
        code.writeline(
            "offset0 = (pid0 * BLOCK_SIZE0 + tl.arange(0, BLOCK_SIZE0)[:, None]).to(tl.int64)"
        )
        if inp_rank == indices_len:
            code.writeline(
                "offset1 = (pid1 * 1 + tl.arange(0, 1)[None, :]).to(tl.int64)"
            )
        else:
            code.writeline(
                "offset1 = (pid1 * BLOCK_SIZE1 + tl.arange(0, BLOCK_SIZE1)[None, :]).to(tl.int64)"
            )
        code.newline()
        ...
        for i in range(indices_len):
            # Indices use standard pointer arithmetic but forced to int64
            comp = [f"indices_idx{j} * indices{i}_stride{j}" for j in range(index_rank)]
            code.writeline(
                f"cur_index{i} = tl.load(indices{i}_ptr + ({' + '.join(comp)}).to(tl.int64), mask=mask0, other=0)"
            )
        code.newline()
        index_mask = [
        ...
        # Manual Byte Address Calculation for Input
        # Note: input_stride{i} are passed as BYTES, and cur_index/input_idx are int64.
        comp = [
            f"cur_index{i}.to(tl.int64) * input_stride{i}" for i in range(indices_len)
        ]
        comp += [
            f"input_idx{i} * input_stride{i}" for i in range(indices_len, inp_rank)
        ]
        code.writeline(f"input_offset = {' + '.join(comp)}")
        # Manual Byte Address Calculation for Output
        ...
        # Address reconstruction: uint64_ptr + byte_offset -> cast back to pointer type
        code.writeline(
            f"cur_value = tl.load((input_ptr + input_offset).to(tl.pointer_type({dtype_str})), mask = mask)"
        )
        code.writeline(
            f"tl.store((out_ptr + out_offset).to(tl.pointer_type({dtype_str})), cur_value, mask=mask)"
        )
...
def generate_index_wrapper(
    ...
    with code.indent():
        ...
        # Calculate strides in BYTES for manual addressing
        code.writeline("element_size = input.element_size()")
        code.writeline("input_stride = [s * element_size for s in input.stride()]")
        ...
        # Calculate strides in BYTES for manual addressing
        code.writeline("out_stride = [s * element_size for s in out.stride()]")
...
def generate_code(
    ...
    # Capture dtype for explicit pointer casting
    dtype = inputs[0].dtype
    dtype_str = str(dtype).replace("torch.", "tl.")

    code = generate_imports(code)
    generate_index_kernel(
        inp_rank, indices_len, index_rank, kernel_name, code, dtype_str
    )
...
class IndexFunction:
    ...
    def arg_key(self, *args, **kwargs):
        ...
        if indices_len == 0:
            ...
            index_rank = tensor_indices[0].ndim
        # Include dtype in cache key because kernel now hardcodes type casting
        return f"inp_rank_{inp_rank}_indices_len_{indices_len}_index_rank_{index_rank}_dtype_{inp.dtype}"
